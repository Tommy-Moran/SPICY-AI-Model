...
Architecture:
  - Base model  : DeepSeek-R1-Distill-Qwen-14B (Ollama tag deepseek-r1:14b,
                  on-premises, Q4_K_M quantisation)
  - Embeddings  : sentence-transformers (all-MiniLM-L6-v2)
  - Vector store: FAISS
  - Framework   : LangChain

Knowledge base (see knowledge_base/README.md):
  - 2021 AHA/ACC/ASE/CHEST/SAEM/SCCT/SCMR Chest Pain Guideline
  - 2021 ACC/AHA/SCAI Coronary Artery Revascularization Guideline
  - 2021 Canadian Cardiovascular Society Dyslipidemia Guideline
...
OLLAMA_MODEL   = "deepseek-r1:14b"  # DeepSeek-R1-Distill-Qwen-14B
...
def validate_plan(plan: str) -> list[str]:
    ...
    if re.search(r"\binvent\b|\bassume\b|I\s+don['\u2019]t\s+have|\bI cannot\b", plan, re.I):
...
def _embeddings(embed_model: str) -> HuggingFaceEmbeddings:
    """Return the pinned sentence-transformer embedding model."""
...
def build_vectorstore(docs: list, embed_model: str, store_path: Path) -> FAISS:
    ...
    embeddings = _embeddings(embed_model)
...
def load_or_build_vectorstore(kb_dir: Path, embed_model: str, store_path: Path) -> FAISS:
    ...
    if store_path.exists():
        ...
        # allow_dangerous_deserialization is required by LangChain-FAISS when loading a
        # locally built index from a trusted path. The index is generated by this codebase
        # only; never load an index from an untrusted or user-supplied source.
